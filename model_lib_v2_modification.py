...
def train_loop(
  ...
  with strategy.scope():
    ...
    def train_dataset_fn(input_context):
      ...
      """Creates a training dataset with image filenames included."""
      ## 26.02.2025 added by TTS to get image fileanames from training dataset 
      ...
      def _parse_function(proto):
          ...
          image_data = tf.io.decode_jpeg(parsed_features['image/encoded'], channels=3)

          return parsed_features['image/filename'], image_data

      tfrecord_dataset = raw_dataset.map(_parse_function)
      tfrecord_dataset = tfrecord_dataset.batch(train_config.batch_size)
      tfrecord_dataset = tfrecord_dataset.repeat() 
      tfrecord_dataset = tfrecord_dataset.prefetch(tf.data.experimental.AUTOTUNE)

      # Create train_input dataset
      ...
      # Combine datasets with filenames
      def _combine_features(filenames, image_data, train_features, train_labels):
          # Ensure filenames and image data are paired
          train_features['image_filenames'] = filenames
          train_features['image_data'] = image_data
          return train_features, train_labels
  
      combined_dataset = tf.data.Dataset.zip((tfrecord_dataset, train_input))
      combined_dataset = combined_dataset.map(
          lambda tfrecord_data, train_data: _combine_features(
              tfrecord_data[0], tfrecord_data[1], train_data[0], train_data[1]
          )
      )
  
      # Prefetch the combined dataset for better performance
  ...
  with summary_writer.as_default():
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        ckpt.restore(latest_checkpoint)  # Ensure checkpoint restoration
        
        @tf.function
        def train_step_fn(features, labels, log_summaries=False):
            """Single training step function with filenames."""
            ## 26.02.2025 added by TTS to read an image data in a tensorboard.
            if log_summaries and record_summaries:
                # Log images to TensorBoard
                tf.compat.v2.summary.image(
                    name='train_input_images',
                    step=global_step,
                    data=features['image_data'],  # Use image_data here
                    max_outputs=3  # Log up to 3 images per step
                )
            ...
            # Debug: Print the filename and global step
            # tf.print("TensorBoard Filename: ", features['image_filenames'], " Global Step: ", int(global_step.value()))
            return losses_dict, features

        def _sample_and_train(strategy, train_step_fn, data_iterator, log_summaries=False):
            # 26.02.2025 added by TTS to extract an input image file name.
            """Runs a training step and extracts filenames correctly."""
            ...
            if isinstance(data, tuple) and len(data) == 3:
                ...
                raise ValueError(f"Unexpected data structure: {type(data)}, {len(data)} elements")
            per_replica_losses_dict, per_replica_features = strategy.run(
                train_step_fn, args=(features, labels, log_summaries))
            return reduce_dict(strategy, per_replica_losses_dict, tf.distribute.ReduceOp.SUM), per_replica_features
        
        @tf.function
        def _dist_train_step(data_iterator, log_summaries=False):
            """ Distributed training step function. """
            # 26.02.2025 added by TTS to valid log summary for image tensorboard on the first step of the iteration.
            if num_steps_per_iteration > 1:
                for i in range(num_steps_per_iteration - 1):
                    with tf.name_scope(''):
                        # Only log summaries on the first step of the iteration
                        _sample_and_train(strategy, train_step_fn, data_iterator, log_summaries=(i == 0))
            
            # Log summaries on the last step of the iteration
            return _sample_and_train(strategy, train_step_fn, data_iterator, log_summaries=log_summaries)

        def write_csv_row(csv_filepath, step_number, image_filename, loss_value):
            file_exists = os.path.isfile(csv_filepath)
            with open(csv_filepath, mode='a', newline='', encoding='utf-8-sig') as csv_file:
                writer = csv.writer(csv_file)
                if not file_exists:
                    writer.writerow(["Step Number", "Image File Name", "Loss"])
                # Write the data row
                writer.writerow([step_number, image_filename, loss_value])

        train_input_iter = iter(train_input)
        if int(global_step.value()) == 0:
          manager.save()

        csv_filepath = os.path.join(model_dir, "training_log.csv")
        ...
        for _ in range(global_step.value(), train_steps, num_steps_per_iteration):
          # 26.02.2025 added by TTS to extract and save an image and input image file in each iteration from a tensorboard. 
          # Log summaries only on the first iteration step
          log_summaries = (global_step.value() % num_steps_per_iteration == 0)
          losses_dict, features = _dist_train_step(train_input_iter, log_summaries=log_summaries)
          
          # Define the directory to save images
          # trained_step_folder = "trainedStep"
          # step_folder = os.path.join(model_dir, trained_step_folder)
          # os.makedirs(step_folder, exist_ok=True)
          image_filename = features['image_filenames'].numpy()[0].decode("utf-8")
          # saved_image_filename = f"TensorImg{tensorNo}_{int(global_step.value())}_{image_filename}.png"
          # image_path = os.path.join(step_folder, saved_image_filename)
          # tensorNo += 1
          
          # Ensure you are saving the correct image tensor
          image_data = features['image_data']  # This is the same tensor used for TensorBoard

          # Save the image
          # image = tf.cast(image_data * 255, tf.uint8)  # image data is in the correct range
          # image = tf.image.encode_png(tf.squeeze(image, axis=0))
          # tf.io.write_file(image_path, image)
          if log_summaries:
              tf.compat.v2.summary.image(
                  name='train_input_images',
                  step=global_step,
                  data=image_data,  # Log the image tensor used in the loop
                  max_outputs=3
              )

          # tf.print("For loop  Filename: ",saved_image_filename , " Global Step: ", int(global_step.value()))
          write_csv_row(csv_filepath, int(global_step.value()), image_filename, float(losses_dict['Loss/total_loss'].numpy()))
          ...
          if global_step.value() - logged_step == num_steps_per_iteration:
            ...
            tf.logging.info(
                f"tensorflow Version:{str_tf_version} |  Wait:{wait} | Step:{global_step.value()} | step_time:{time_taken:.3f}s | avg_time:{average_time:.4f}s | total_time:{total_time / 3600:.5f}h")
            tf.logging.info(pprint.pformat(logged_dict_np, width=40))
            logged_step = global_step.value()
            i += 1
            total_loss_list.append(float(losses_dict['Loss/total_loss'].numpy()))
            if int(global_step.value()) >= 15000:   # 원래 가치 35000 
            ...
            if wait >= patience:
              ...
              print(f"It's done. Do NOT need to update Best Loss")
              sys.exit()
          tf.logging.info(f"Global Step: {int(global_step.value())}")
          if int(global_step.value()) >= 15000:   # 원래 가치  35000
            if int(global_step.value()) % checkpoint_every_n == 0:
              if total_loss_list[i] < 0.1:
                manager.save(int(global_step.value()/checkpoint_every_n))
  ...
  # Export performance metrics if provided
